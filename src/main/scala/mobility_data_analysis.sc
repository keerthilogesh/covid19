...
val window_month =  Window.partitionBy("CC", "Country").orderBy(asc("Date")).rowsBetween(-30, 0)
...
// plot luxembourg daily cases data
...
val paramGrid_glr = new ParamGridBuilder().addGrid(glr.family, Array("tweedie", "poisson")).addGrid(glr.link, Array("identity", "log", "logit")).addGrid(glr.maxIter, Array(50)).addGrid(glr.regParam, Array(0.1, 0.5)).build()
val paramGrid_lr = new ParamGridBuilder().addGrid(glr.maxIter, Array(50)).addGrid(glr.regParam, Array(0.1, 0.5)).build()
...
// Finding the average duration before death or recovery
var duration_recovery = cases_mobility_ds.select("CC","Date", "Confirmed_change", "Recovered_change").cache()
//val window_lag = Window.partitionBy("CC", "Country").orderBy(asc("Date"))
duration_recovery = duration_recovery.withColumn("Confirmed-1", lag("mobility", 1))